...
of $\prod_e x_e^{2j_e}$ defines a canonical coefficient family $C_G(\{j_e\})$ associated with the determinant generating functional. Identifying $C_G$ with a particular Wigner-symbol convention requires fixing (and applying) an explicit normalization and phase map (intertwiner normalization, edge orientations, and Schwinger-boson conventions).
