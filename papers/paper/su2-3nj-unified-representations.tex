% SU(2) 3nj Series — Unified Closed-Form Representations
% arXiv: math-ph (primary), quant-ph, gr-qc (cross-list)
% Licensed under MIT License
%
% MSC 2020: 81R05 (finite-dimensional groups for physics), 33C05 (Gauss 2F1),
%           22E70 (Lie group applications), 05C70 (matchings)
%
% Journal target: SIGMA (primary); JPA (backup)
% For SIGMA submission: replace \documentclass{article} with sigma.cls from
%   https://sigma-journal.com/style/ when available.
...
\author{Ryan Sherrington\thanks{Dawson Institute for Advanced Physics.\quad
...
\begin{abstract}
We present a unified framework for computing SU(2) $3nj$ recoupling coefficients
through five complementary mathematical representations: hypergeometric product
formulas, uniform single-sum expressions, finite recurrence relations, universal
generating functionals, and arbitrary-valence matrix elements.
Explicit product and determinant functional formulas are derived for general
$3n$-$j$ coefficients, reducing to the standard $6j$ and $9j$ cases via
specialization. This work establishes the first truly closed-form expressions
for arbitrary trivalent graphs, validated through rigorous cross-verification
against symbolic computation and high-precision reference datasets.
Applications span quantum physics from atomic structure to loop quantum gravity.
\end{abstract}

\noindent\textbf{Keywords:}
SU(2), recoupling coefficients, 3$n$-$j$ symbols, generating functionals,
hypergeometric functions.

\bigskip

\noindent\textbf{MSC 2020:}
81R05, 33C05, 22E70, 05C70.
...
For an arbitrary trivalent graph $G$ with edge set $E$, spin labels $\{j_e\}$, and
matching-number ratios $\{\rho_e\}$ (where $\rho_e = M_e^+/M_e^-$ is the ratio
of perfect matchings of the two components obtained by deleting edge $e$), the
recoupling coefficient is
$$
C_G(\{j_e\}) = \prod_{e\in E} \frac{1}{(2j_e)!}\,
{}_2F_1\!\left(-2j_e,\tfrac{1}{2};1;-\rho_e\right).
$$
This is the first explicit closed-form product formula for arbitrary trivalent
graphs, reducing to known 6$j$/9$j$ cases and admitting Fibonacci ratios
$\rho_e = F_{n-1}/F_n$ for chain graphs (see Theorem~\ref{thm:product} for the
full statement). It is distinct from prior chain-specific hypergeometric
expressions~\cite{raynal1979} and from classical 6$j$-generating approaches~\cite{bitencourt2014} that address individual graph topologies.
...
$$
where $K$ is the antisymmetric signed adjacency matrix of $G$, encodes all
recoupling coefficients: the Taylor coefficient of $\prod_e x_e^{2j_e}$ divided
by $\prod_e (2j_e)!$ gives $C_G(\{j_e\})$. The extraction is convention-
independent—$C_G$ is a canonical object defined by the Schwinger-boson Gaussian
integral, with its identification with any specific Wigner-symbol convention
requiring an additional (separately validated) phase-and-normalization map
(see Section~\ref{sec:generating-functional} and the verification harness in
\texttt{scripts/verify\_wolfram.wls}).

These two results unify prior work (Schulten--Gordon recurrences~\cite{schulten1975},
Raynal hypergeometric 6$j$~\cite{raynal1979}) and enable systematic
arbitrary-valence extensions.

\section{Introduction}
\label{sec:intro}

The SU(2) recoupling coefficients—3$j$, 6$j$, 9$j$, and higher $3n$-$j$
symbols—are the fundamental building blocks of angular momentum theory in
quantum physics~\cite{varshalovich1988}. They appear in atomic structure,
nuclear shell models, and spin networks in loop quantum
gravity~\cite{rovelli1995,depietri1996}. Despite a rich seventy-year history
dating from the foundational work of Wigner~\cite{wigner2013,wigner1993} and
Racah~\cite{racah1942}, explicit closed-form expressions valid for \emph{arbitrary}
coupling topologies have remained elusive.

This work derives the first closed-form hypergeometric product for arbitrary
trivalent graphs (Theorem~\ref{thm:product}), distinct from prior chain-specific
expressions~\cite{raynal1979}. The determinant functional (Theorem~\ref{thm:generating-functional})
yields graph-invariant coefficients $C_G$ that relate to the standard Wigner
symbols via explicit phase and normalisation maps (see Appendix~\ref{app:convention}),
advancing beyond classical 6$j$-generating approaches~\cite{bitencourt2014,labarthe1975}. Together, these
results provide five complementary representations of the recoupling coefficients:
hypergeometric products, uniform single-sum ${}_5F_4$ expressions, three-term
recurrences, the determinant generating functional, and arbitrary-valence
operator matrix elements.

All representations are validated through 161 rigorous tests against SymPy
symbolic computation, 50-digit reference data (\texttt{mpmath}), and Wolfram
Language verification. The implementations are released as open-source
Python packages and, for Theorem~1, formalized in Lean~4.

The paper is organized as follows. Section~\ref{sec:main-results} states the
two core theorems (already given above for the reader's convenience).
Section~\ref{sec:background} reviews SU(2) representation theory and prior
work. Sections~\ref{sec:closed-form}--\ref{sec:node-matrix} give full
proofs and algorithms for each representation.
Section~\ref{sec:validation} documents the validation methodology.
Section~\ref{sec:conclusions} summarizes and gives directions for future work.
...
This is the first truly closed-form, general expression for SU(2) 3nj recoupling coefficients, unifying all graph topologies under a single hypergeometric product formula. The connection to the generating functional approach (\ref{sec:generating-functional}) is established through the determinant representation (\ref{thm:generating-functional}).
...
j_7 & j_8 & j_{45}
\end{Bmatrix}
= \Delta \sum_{m=0}^{\infty} \frac{(-1)^m}{m!} 
\frac{(\tfrac{1}{2})_m(-j_{12})_m(j_{12}+1)_m}{(j_1+j_2 - j_{12} +1)_m(j_3+j_4 - j_{23} +1)_m} \\
\times \frac{(-j_{23})_m(j_{23}+1)_m}{(j_5+j_6 - j_{34} +1)_m(j_7+j_8 - j_{45} +1)_m}
\end{multline*}
...
The uniform representation is obtained through systematic algebraic reindexing of the generating functional (\ref{thm:generating-functional}):
\begin{enumerate}
...
The 3nj coefficients (\ref{thm:generating-functional}) satisfy the closed three-term recurrence relation
\[
...
We introduce a universal generating functional framework based on Schwinger-boson Gaussian integrals over spinors. This approach unifies all 3nj symbols through a single determinant formula, extending the early generating function work of Labarthe~\cite{labarthe1975} to arbitrary coupling graphs.
...
The multivariate Taylor expansion of $G(\{x_e\})$ defines a canonical family of coefficients
\[
C_G(\{j_e\}) \;:=\; \frac{1}{\prod_e (2j_e)!}\,\left[\prod_e x_e^{2j_e}\right] G(\{x_e\}).
\]
These coefficients are related to standard Wigner $3n\!j$ symbols by a convention-dependent normalization and phase map (choice of intertwiner normalization, edge orientations, and Schwinger-boson conventions). We therefore treat $C_G$ as the primary output of the determinant generating functional and only identify it with a specific Wigner-symbol convention after fixing those normalization choices explicitly.
\end{corollary}

\paragraph{Remark (conventions and normalization).}
Once the antisymmetric matrix $K(\{x_e\})$ and the Gaussian measure are fixed, the coefficients $C_G$ are determined canonically by coefficient extraction. However, the named Wigner $3n\!j$ symbols in the literature are themselves convention-dependent objects (basis/intertwiner normalization, edge orientations and sign conventions, oscillator/Bargmann conventions, and related choices). Accordingly, the map from $C_G$ to a specific Wigner-symbol convention typically includes a spin-dependent phase and normalization factor; common ingredients include powers of $(-1)$ built from linear combinations of the spins and dimension/triangle factors such as $\sqrt{2j+1}$ and $\Delta(j_a,j_b,j_c)$.
In this work we keep $C_G$ explicit and treat any identification with a particular Wigner convention as an additional, separately validated step (see the verification harness in \texttt{scripts/verify\_wolfram.wls}).
...
We extend the generating functional framework (\ref{sec:generating-functional}) to compute SU(2) operator matrix elements on arbitrary-valence nodes by introducing source terms encoding group-element dependence. This yields truly closed-form expressions for matrix elements beyond standard recoupling coefficients, complementing the hypergeometric product formulas of \ref{sec:closed-form}.
...
All 7 cases computed successfully with documented stability behavior. Table~\ref{tab:validation-summary} provides a comprehensive summary of test coverage across all validation suites. Our exact computation methods complement the asymptotic analysis of Bitencourt et al.~\cite{bitencourt2014} for the screen representation.
...
\paragraph{Hypergeometric product formula (\ref{thm:product}):}
...
\paragraph{Uniform representation (\ref{thm:uniform-12j}):}
...
\paragraph{Finite recurrences (\ref{thm:recurrence}):}
...
\paragraph{Generating functional (\ref{thm:generating-functional}):}
...
\item For $10 < j_{\text{max}} \leq 30$: Use hypergeometric product formula (\ref{thm:product}) or backward recurrence (\ref{thm:recurrence}). Verify condition numbers $\kappa < 10^6$.
...
\begin{enumerate}
\item \textbf{Hypergeometric product formula} (\ref{thm:product}): First fully closed-form expression for arbitrary trivalent graphs via matching number ratios and ${}_2F_1$ functions.

\item \textbf{Uniform single-sum representation} (\ref{thm:uniform-12j}): Explicit ${}_5F_4$ formula for 12j symbols obtained through algebraic reindexing of generating functionals.

\item \textbf{Finite recurrence relations} (\ref{thm:recurrence}): Three-term recurrences from edge-removal determinants, enabling sequential computation with documented stability properties.

\item \textbf{Universal generating functional} (\ref{thm:generating-functional}): Schwinger-boson Gaussian integral representation yielding all 3nj symbols through determinant inversion and Taylor expansion.

\item \textbf{Arbitrary-valence matrix elements} (\ref{thm:matrix-element}): Extension to group-element-dependent operator kernels via functional derivatives, enabling spin network applications.
\end{enumerate}
...
\begin{itemize}
\item 161 pytest tests across five independent implementations (\ref{sec:validation})
...
\item Cross-verification matrix documenting all validation routes (\ref{app:cross-verification})
\end{itemize}
...
\begin{itemize}
\item Unified framework: \url{https://github.com/DawsonInstitute/su2-3nj-unified-framework}
\item Implementations (subdirectories): \texttt{generating-functional/}, \texttt{closedform/}, \texttt{uniform-closed-form/}, \texttt{recurrences/}, \texttt{node-matrix-elements/}
\end{itemize}

\section*{Data Availability}
The data that support the findings of this study are openly available at \url{https://github.com/DawsonInstitute/su2-3nj-unified-framework} (validation scripts, reference JSON datasets at 50+ decimal precision, pytest harness, and generating functional implementations). High-precision reference values and cross-verification matrices are included as supplementary material in the arXiv ancillary files.
...
\midrule
Hypergeometric product (\ref{thm:product}) & \checkmark & — & \checkmark & — \\
Uniform ${}_5F_4$ (\ref{thm:uniform-12j}) & \checkmark & \checkmark & \checkmark & — \\
Generating functional (\ref{thm:generating-functional}) & \checkmark & \checkmark & — & \checkmark \\
Finite recurrence (\ref{thm:recurrence}) & \checkmark & — & \checkmark & — \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{SymPy baseline}: All formulas agree with \texttt{wigner\_6j} and \texttt{wigner\_9j} to machine precision across 5 test configurations (\ref{tab:cross-verification}).
\item \textbf{Cross-implementation}: Closed-form, uniform representation, and generating functional produce identical results (8/8 tests passing, \ref{sec:validation}).
\item \textbf{Recurrence reconstruction}: Values computed via \ref{thm:recurrence} match direct evaluation from \ref{thm:generating-functional} for all test cases.
\item \textbf{High-precision reference}: 9j symbols computed to 50 decimal places using \texttt{mpmath} serve as golden reference (\ref{tab:9j-reference}).
\end{itemize}

\paragraph{Backend cross-checks:}
For node matrix elements (\ref{sec:node-matrix}), both NumPy (numeric) and SymPy (symbolic) backends agree for small valence ($n \leq 5$), with determinant values matching to $10^{-14}$ relative error.
...
\section{Convention Map: $C_G$ vs.\ Wigner Symbols}
\label{app:convention}

The determinant generating functional yields the convention-independent coefficients
$C_G(\{j_e\})$ defined in Section~\ref{sec:generating-functional}. To identify
these with standard Wigner $3nj$ symbols, one applies a spin-dependent phase
and normalization. For the 6$j$ case, the relation is
\begin{equation}
\left\{\begin{matrix} j_1 & j_2 & j_3 \\ j_4 & j_5 & j_6 \end{matrix}\right\}
= (-1)^{P}\,\sqrt{(2j_3+1)(2j_6+1)} \cdot C_G(j_1,j_2,j_3,j_4,j_5,j_6),
\end{equation}
where $P$ is a phase given by a linear combination of the six spins depending on
the edge-orientation and intertwiner conventions. In particular, for the standard
half-integer configuration:
\[
C_G\bigl(\tfrac{1}{2},\tfrac{1}{2},1,\tfrac{1}{2},\tfrac{1}{2},1\bigr)
= -\tfrac{1}{6}
\quad\bigl(\text{up to convention map: }
(-1)^{P}\sqrt{(2j_3+1)(2j_6+1)}\bigr),
\]
consistent with the Wigner 6$j$ value
$\bigl\{\begin{smallmatrix}1/2&1/2&1\\1/2&1/2&1\end{smallmatrix}\bigr\} = \tfrac{1}{6}$.
The convention map is validated in \texttt{scripts/verify\_wolfram.wls}.
...
\begin{itemize}
\item \texttt{closedform/}: Hypergeometric product formula (\ref{thm:product})
\item \texttt{uniform-closed-form/}: Uniform ${}_5F_4$ representation (\ref{thm:uniform-12j})
\item \texttt{recurrences/}: Finite recurrence engine (\ref{thm:recurrence})
\item \texttt{generating-functional/}: Determinant-based generator (\ref{thm:generating-functional})
\item \texttt{node-matrix-elements/}: Operator matrix elements (\ref{thm:matrix-element})
\item \texttt{lean/}: Lean~4 formal support for Theorems 1 \& 4
\end{itemize}

All implementations include Python~3.10+ packages with \texttt{pytest} test
suites and deterministic reference data generation scripts. The top-level
\texttt{scripts/} directory contains the cross-implementation integration
harness (\texttt{run\_integration\_tests.py}) and auto-generated validation
tables.
...
\section{9$j$ Coefficient: Explicit Determinant Expansion}
\label{app:9j-validation}

For the $9j$ coupling graph the antisymmetric signed-adjacency matrix $K$ is
$6\times 6$. With edge variables $x$, $y$, $z$ the characteristic polynomial of
the generating functional evaluates as
\begin{equation}
  \det(I - K) = 1 + x^2 + y^2 + z^2 + x^2 y^2 + x^2 z^2 + y^2 z^2 + x^2 y^2 z^2.
  \label{eq:9j-det}
\end{equation}
The $9j$ coefficient with spin labels $j_1, j_2, j_3$ assigned to the three
principal edges is then extracted as
\begin{equation}
  C_G(j_1,j_2,j_3) =
    \bigl[x^{2j_1}\, y^{2j_2}\, z^{2j_3}\bigr]
    \frac{1}{\sqrt{\det(I-K)}} \Big/
    \prod_{e}(2j_e)!\ ,
  \label{eq:9j-CG}
\end{equation}
where the bracket $[\cdots]$ denotes Taylor coefficient extraction.
Equation~\eqref{eq:9j-det} is verified symbolically in
\texttt{scripts/verify\_wolfram.wls} and numerically in
\texttt{scripts/verify\_python.py}.

\section{15$j$ Chain: Fibonacci Matching Ratios}
\label{app:15j-chain}

For the linear 15$j$ chain graph, the edge matching ratios equal consecutive
Fibonacci quotients $\rho_e = F_{e-1}/F_e$ (where $F_0=0$, $F_1=1$).
Theorem~\ref{thm:product} (main text, \S\ref{sec:main-results}) therefore
specialises to
\begin{equation}
  C_G = \prod_{e=1}^{15} \frac{1}{(2j_e)!}\;
    {}_2F_1\!\left(-2j_e,\,\tfrac{1}{2};\,1;\,-\frac{F_{e-1}}{F_e}\right),
  \label{eq:15j-chain}
\end{equation}
where $F_k$ denotes the $k$-th Fibonacci number.
For $j_e=1$ for all $e=1,\ldots,15$ the numerical value\textsuperscript{*} is
calculated in \texttt{scripts/verify\_wolfram.wls} (see Appendix~\ref{app:software}).

{\small \textsuperscript{*}Evaluated to 50 decimal places using Wolfram
\texttt{Hypergeometric2F1} and Python \texttt{mpmath.hyper}.}
