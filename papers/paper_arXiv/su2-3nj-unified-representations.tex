...
\address[R.~Sherrington]{Dawson Institute for Advanced Physics}
\email{\href{mailto:rsherrington@dawsoninstitute.org}{rsherrington@dawsoninstitute.org}}

\date{February 1, 2026. arXiv v1}

\subjclass[2020]{81R05, 33C45, 05C90, 81T45}
...
\label{thm:product}
Let $G$ be a connected trivalent \emph{tree} (i.e., an acyclic connected graph) with edge set $E$. Label each edge $e \in E$ by a spin $j_e$. For each edge $e$, delete $e$ from $G$, obtaining two connected components (subtrees) $G_e^+$ and $G_e^-$. Let $M_e^+$ and $M_e^-$ be the number of \emph{matchings} (independent edge sets, not necessarily perfect) of $G_e^+$ and $G_e^-$, respectively. Define the matching ratio
\[
\rho_e = \frac{M_e^+}{M_e^-}.
\]
...
For the 15j-chain on 8 vertices (7 edges), cutting edge $e$ splits the chain into two paths. The number of matchings of a path graph $P_n$ on $n$ vertices is $M(P_n)=F_{n+2}$, so the resulting matching counts are Fibonacci numbers, giving
\[
...
The core computational bottleneck is computing matching numbers $M_e^{\pm}$ for edge deletions. For trees (the setting of \Cref{thm:product}), total matchings can be computed in linear time via dynamic programming on the components $G_e^{\pm}$.

\begin{quote}
\textbf{Algorithm 1}: Matching-number computation on trees
\begin{enumerate}
\item \textbf{Input}: Graph $G$, edge $e$
\item Delete edge $e$ from $G$ to obtain $G_e^+$ and $G_e^-$
\item For each component:
\begin{enumerate}
    \item[(a)] Root the tree and compute the number of matchings by standard DP recurrences
    \item[(b)] (Optional) For path components, use the closed form $M(P_n)=F_{n+2}$
\end{enumerate}
...
For general (non-tree) trivalent graphs, both the edge-deletion connectivity properties and the combinatorics of matchings require additional case distinctions; we do not rely on Pfaffian perfect-matching formulas for total matchings in this work.
...
{(j_1+j_2 - j_{12} +1)_m(j_3+j_4 - j_{23} +1)_m(j_5+j_6 - j_{34} +1)_m(j_7+j_8 - j_{45} +1)_m} \\
...
Starting from the tetrahedral $4\times 4$ antisymmetric adjacency matrix $K$ with six edge variables, a simple two-parameter specialization is obtained by identifying three opposite edges with $x$ and the remaining three with $y$ (i.e., $a=b=c=x$ and $d=e=f=y$ in the tetrahedral labeling). This yields
\[
\det(I-K)\big|_{a=b=c=x,\,d=e=f=y} = 1 + 3x^2 + 3y^2 + x^2 y^2,
\]
and hence
\[
G(x,y) = \frac{1}{\sqrt{1 + 3x^2 + 3y^2 + x^2 y^2}}.
\]
...
The data that support the findings of this study are openly available in the Dawson Institute GitHub repositories at \url{https://github.com/DawsonInstitute/su2-3nj-series-paper} and related \texttt{su2-3nj-*} repositories.

\begin{suppinfo}
Supplementary material includes the full validation harness, reproducible test datasets, and scripts used to generate the tables included in this manuscript.
\end{suppinfo}
